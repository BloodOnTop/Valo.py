  ...
  ▒██ █░░   ░██▄▄▄▄██    ▒██░       ▒██   ██░           ▒██▄█▓▒ ▒ ░ ▐██▓░
   ...
   """
    ...
    def ScreenLOG():
        ...
        cmp1 = cpm
        cpm = 0
        windll.kernel32.SetConsoleTitleW(f" Valo.py - (By Blood) | Checked: {checked}\{len(emails)} -  Good: {hits} - Bad: {fail} - Customs: {free} - Banned Accounts: {banned} - Cpm: {cmp1*60} - Retries: {retries} - Errors: {error} ")
        time.sleep(1)
    ...
    def CheckerMenu():
        ...
        clear()
        windll.kernel32.SetConsoleTitleW("Valo.py - (By Blood)")
        print(logo)
        print()
        print()
        try:
